...
def generate_path(points_list):
    ...
    # Compute distance between each park, including the DUMP
    park_distances = compute_distance_matrix(park_coords)

    # Solve the TSP for parks with SHOP as the start
    tsp_indices = solve_tsp(park_distances)

    # Create optimal order of parks excluding DUMP
    optimal_park_order = [park_ids[i]
                          for i in tsp_indices if i < len(park_ids)]
...
def cluster_points_into_parks(points_list):
    ...
    distance_matrix = squareform(
        pdist(points_list, metric=lambda u, v: haversine(u, v, unit=Unit.KILOMETERS)))

    # Perform DBSCAN clustering
    db = DBSCAN(eps=WALKING_DISTANCE_THRESHOLD_KM,
